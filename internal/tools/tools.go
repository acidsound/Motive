...
- If a previous run in this session was interrupted or failed before finishing,
  ...
  first, then use session_log if relevant; decide yourself what to do next.
  Full observational logs and persisted reasoning are not recovery context.`
...
func Definitions() []model.Tool {
	...
	return []model.Tool{
		...
		{Type: "function", Function: model.ToolFunction{Name: "session_log", Description: "Read the last N entries of a session's .jsonl transcript (default: the current session).", Parameters: obj(map[string]model.ToolProperty{"lines": {Type: "integer", Description: "Number of trailing transcript entries to return (default 5, max 20)"}, "session_id": {Type: "string", Description: "Session id to read; defaults to the current session"}})}},
