...
func New(client *model.Client) *Runtime {
	...
	return &Runtime{Model: client, WS: ws, Exec: &tools.Executor{WS: ws}, Observ: observation.New(), MaxSteps: steps, Budget: ExecutionBudget{MaxSteps: steps, MaxDuration: time.Duration(minutes) * time.Minute, MaxToolCalls: toolCalls}}
}
...
func (r *Runtime) ContextBlock() string {
	...
	if statusErr == nil && status != "" { b.WriteString("\nGit status:\n"); b.WriteString(status) }
	if len(files) > 6000 { files = truncateUTF8(files, 6000) + "\n..." }
...
func (r *Runtime) emit(event TraceEvent) { if r.Trace != nil { r.Trace(event) } }
...
func (r *Runtime) Execute(ctx context.Context, request string) (string, error) {
	...
	trace := []string{}
	budget := r.Budget
	if budget.MaxSteps <= 0 { budget.MaxSteps = r.MaxSteps }
	if budget.MaxSteps > maxAllowedSteps { budget.MaxSteps = maxAllowedSteps }
	if budget.MaxDuration <= 0 { budget.MaxDuration = defaultMaxMinutes * time.Minute }
	if budget.MaxDuration > maxAllowedMinutes*time.Minute { budget.MaxDuration = maxAllowedMinutes * time.Minute }
	if budget.MaxToolCalls <= 0 { budget.MaxToolCalls = defaultMaxToolCalls }
	if budget.MaxToolCalls > maxAllowedToolCalls { budget.MaxToolCalls = maxAllowedToolCalls }
	...
	for step := 0; step < budget.MaxSteps; step++ {
		if err := ctx.Err(); err != nil { r.emit(TraceEvent{Kind: "finish", Step: step, MaxSteps: budget.MaxSteps, TotalToolCalls: obs.TotalToolCalls, TotalElapsed: time.Since(started), ReasoningEffort: effort, BaseRevision: baseRevision, ResultRevision: r.WS.GitHEAD(), Error: err}); return "", err }
		stepNumber := step + 1
		...
		var err error
		if r.Stream {
			msg, stats, err = r.Model.ChatStream(ctx, messages, toolDefs, effort, func(delta model.StreamDelta) { r.emit(TraceEvent{Kind: "delta", Step: stepNumber, MaxSteps: budget.MaxSteps, MessageCount: len(messages), TotalToolCalls: obs.TotalToolCalls, ReasoningEffort: effort, TotalElapsed: time.Since(started), Text: delta.Content, Reasoning: delta.Reasoning}) })
		} else { msg, stats, err = r.Model.ChatWithEffort(ctx, messages, toolDefs, effort) }
		if err != nil { r.emit(TraceEvent{Kind: "model_end", Step: stepNumber, MaxSteps: budget.MaxSteps, MessageCount: len(messages), TotalToolCalls: obs.TotalToolCalls, RequestBytes: stats.RequestBytes, EstimatedInputTokens: stats.EstimatedInputTokens, ResponseBytes: stats.ResponseBytes, Latency: stats.Latency, ServerTimings: stats.ServerTimings, ReasoningEffort: effort, TotalElapsed: time.Since(started), Error: err}); r.emit(TraceEvent{Kind: "finish", Step: stepNumber, MaxSteps: budget.MaxSteps, TotalToolCalls: obs.TotalToolCalls, TotalElapsed: time.Since(started), ReasoningEffort: effort, BaseRevision: baseRevision, ResultRevision: r.WS.GitHEAD(), Error: err}); return "", err }
		obs.LastModelLatency = stats.Latency
		if stats.ServerTimings != nil { obs.LastPredictedMS = stats.ServerTimings.PredictedMS; obs.LastPredictedN = stats.ServerTimings.PredictedN }
		...
		if len(msg.ToolCalls) == 0 {
			if msg.Content == "" { err := fmt.Errorf("model finished without a response"); r.emit(TraceEvent{Kind: "finish", Step: stepNumber, MaxSteps: budget.MaxSteps, TotalToolCalls: obs.TotalToolCalls, TotalElapsed: time.Since(started), ReasoningEffort: effort, BaseRevision: baseRevision, ResultRevision: r.WS.GitHEAD(), Error: err}); return "", err }
		...
		for _, call := range msg.ToolCalls {
			if obs.TotalToolCalls >= budget.MaxToolCalls { err := fmt.Errorf("execution budget exceeded: %d tool calls", budget.MaxToolCalls); r.emit(TraceEvent{Kind: "finish", Step: stepNumber, MaxSteps: budget.MaxSteps, TotalToolCalls: obs.TotalToolCalls, TotalElapsed: time.Since(started), ReasoningEffort: effort, BaseRevision: baseRevision, ResultRevision: r.WS.GitHEAD(), Error: err}); return "", err }
			toolStarted := time.Now()
			var result string
			if call.Function.Name == "" { result = "ERROR: tool call has an empty name and was ignored"; toolFailed = true } else if result, err = r.Exec.Run(ctx, call.Function.Name, call.Function.Arguments); err != nil { result = "ERROR: " + err.Error(); toolFailed = true }
			obs.TotalToolCalls++
			if strings.HasPrefix(result, "ERROR: ") { obs.ToolFailures++; r.Observ.ObserveToolFailure() }
			if call.Function.Name == "read_file" && !strings.HasPrefix(result, "ERROR: ") { r.Observ.ObserveFile(toolArgument(call.Function.Arguments, "path"), result, stepNumber) }
			if strings.HasPrefix(result, "ERROR: ") || call.Function.Name == "shell" { r.Observ.ObserveDiagnostic(result) }
		...
		if toolFailed { effort = "xhigh" } else { effort = r.Model.GetReasoningEffort() }
	}
...
func toolArgument(raw, key string) string {
	...
	if err := json.Unmarshal([]byte(raw), &args); err != nil { return "" }
