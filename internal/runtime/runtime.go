...
func New(client *model.Client) *Runtime {
	...
	return &Runtime{Model: client, WS: ws, Exec: &tools.Executor{WS: ws}, MaxSteps: steps, Budget: ExecutionBudget{MaxSteps: steps, MaxDuration: time.Duration(minutes) * time.Minute, MaxToolCalls: toolCalls}}
}
...
func (r *Runtime) Execute(ctx context.Context, request string) (string, error) {
	...
	for step := 0; step < budget.MaxSteps; step++ {
		...
		for _, call := range msg.ToolCalls {
			...
			var result string
			if call.Function.Name == "" { result = "ERROR: tool call has an empty name and was ignored"; toolFailed = true } else {
				var toolErr error
				result, toolErr = r.Exec.Run(ctx, call.Function.Name, call.Function.Arguments)
				if toolErr != nil { result = "ERROR: " + toolErr.Error(); toolFailed = true }
			}
			obs.TotalToolCalls++
			if strings.HasPrefix(result, "ERROR: ") { obs.ToolFailures++ }
