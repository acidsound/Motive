...
def call(v1, model, msgs, tools=None, choice=None, out=64, temp=0.0, timeout=300):
    ...
    t = time.perf_counter(); r = post(v1.rstrip('/') + '/chat/completions', p, timeout)
...
def _run(case,args,prompt,reasoning,tools,tool_result):
    ...
    if case in ('required_tool','roundtrip'):
        ...
        i1=info(r1)
        if i1['calls']==0: return e1,r1,'no_tool_call'
        if case=='required_tool': return e1,r1,'ok'
        m=i1['message']; msgs.append(m)
        c=m['tool_calls'][0]
        msgs.append({'role':'tool','tool_call_id':c.get('id','benchmark'),'content':tool_result})
        msgs.append({'role':'user','content':'The result is available. Do not call another tool. Reply briefly.'})
        e2,r2=call(args.url,args.model,msgs,tools=tools,choice='none',out=args.output_tokens,timeout=args.timeout)
        r2['_first_latency']=e1; r2['_second_latency']=e2
        return e1+e2,r2,'ok'
